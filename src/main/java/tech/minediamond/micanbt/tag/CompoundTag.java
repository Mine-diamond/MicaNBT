...
public abstract class CompoundTag extends Tag implements Iterable<Tag> {
    ...
    @Contract(pure = true)
    public abstract @Nullable Tag get(String tagName);
    ...
    @Contract(pure = true)
    public @Nullable String findKey(Predicate<? super Tag> predicate) {
        for (Tag tag : this) {
    ...
    @Contract(pure = true)
    public @Nullable Tag find(Predicate<? super Tag> predicate) {
        for (Tag tag : this) {
            if (predicate.test(tag)) {
                return tag;
            }
    ...
    public abstract @Nullable Tag remove(String tagName);
